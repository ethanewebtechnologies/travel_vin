...
/*
 * APPLICATION 		: club_and_bar Bookings Controller
...
class club_and_bar_bookings_ctrl extends CI_Controller {
    ...
    public function index() {
        ...
        $search_club_and_bar_start_date = $this->input->get('search_club_and_bar_start_date');
      
        if (isset($search_club_and_bar_start_date)) {
            $search_conditions['search_club_and_bar_start_date'] = trim($search_club_and_bar_start_date);
            $this->_pdata['search_club_and_bar_start_date'] = trim($search_club_and_bar_start_date);
        } else {
            $search_conditions['search_club_and_bar_start_date'] = null;
            $this->_pdata['search_club_and_bar_start_date'] = null;
        }
        ...
        $sort_club_and_bar_start_date = $this->input->get('sort_club_and_bar_start_date');
        
        if (isset($sort_club_and_bar_start_date)) {
            $search_conditions['sort_club_and_bar_start_date'] = trim($sort_club_and_bar_start_date);
            if($sort_club_and_bar_start_date=='ASC'){
                $this->_pdata['sort_club_and_bar_start_date'] = 'entypo-up-open';
            }
            else{
                $this->_pdata['sort_club_and_bar_start_date'] = 'entypo-down-open';
            }
        } else {
            $search_conditions['sort_club_and_bar_start_date'] = null;
            $this->_pdata['sort_club_and_bar_start_date'] = null;
        }
        ...
        $this->load->model('__admin_booking/Club_and_bar_bookings_mdl');
        $club_and_bar_bookings = $this->Club_and_bar_bookings_mdl->get_club_and_bar_bookings($search_conditions, $start, $limit);
        
        $customer_ids_array = array();
        $this->_pdata['club_and_bar_bookings'] = array();
        
        foreach ($club_and_bar_bookings as $club_and_bar_booking) {
            $customer_ids_array[] = $club_and_bar_booking['booking_customer_id'];
            $this->_pdata['club_and_bar_bookings'][] = $club_and_bar_booking;
        }
        
        $customers = $this->Club_and_bar_bookings_mdl->get_customers_name_by_ids($customer_ids_array);
        ...
        $this->_pdata['total_club_and_bar_bookings'] = $this->Club_and_bar_bookings_mdl->get_total_club_and_bar_bookings($search_conditions);
        ...
        $this->template_lib->load_view($this, '__admin_booking/Club_and_bar_booking_list_view', $this->_pdata);
    }
    
    public function get() {
        $secure_token = $this->input->get('secure_token');
        $club_and_bar_booking_id = $this->security_lib->decrypt($secure_token);
        
        $this->load->model('__admin_booking/Club_and_bar_bookings_mdl');
        $this->_pdata['club_and_bar_booking'] = $this->Club_and_bar_bookings_mdl->get_club_and_bar_booking($club_and_bar_booking_id);
        $this->_pdata['text_h3_heading_edit'] = $this->lang->line('text_h3_heading_edit');
        
        $payment_statuses = $this->Club_and_bar_bookings_mdl->get_payment_statuses();
        
        foreach ($payment_statuses as $payment_status) {
            $this->_pdata['payment_statuses'][$payment_status['id']] = $payment_status['name'];
        }
        
        $booking_statuses = $this->Club_and_bar_bookings_mdl->get_booking_statuses();
        
        foreach ($booking_statuses as $booking_status) {
            $this->_pdata['booking_statuses'][$booking_status['id']] = $booking_status['name'];
        }
        
        $this->template_lib->load_view($this, '__admin_booking/Club_and_bar_booking_view', $this->_pdata);
    }
    
    public function confirm_bookings(){
        $this->load->model('__admin_booking/Club_and_bar_bookings_mdl');
        $this->Club_and_bar_bookings_mdl->confirm_bookings($this->input->get('data'));
    ...
    public function cancel_bookings(){
        $this->load->model('__admin_booking/Club_and_bar_bookings_mdl');
        $this->Club_and_bar_bookings_mdl->cancel_bookings($this->input->get('data'));
