...
/*
 * APPLICATION 		: Tour Booking Controller
 * AUTHOR			: KUNDAN KUMAR
...
class Tour_bookings_ctrl extends CI_Controller {
    ...
    public function index() {
        ...
        $search_tour_start_date = $this->input->get('search_tour_start_date');

        if (isset($search_tour_start_date)) {
            $search_conditions['search_tour_start_date'] = trim($search_tour_start_date);
            $this->_pdata['search_tour_start_date'] = trim($search_tour_start_date);
        } else {
            $search_conditions['search_tour_start_date'] = null;
            $this->_pdata['search_tour_start_date'] = null;
        }
        ...
        $sort_tour_start_date = $this->input->get('sort_tour_start_date');
        
        if (isset($sort_tour_start_date)) {
            $search_conditions['sort_tour_start_date'] = trim($sort_tour_start_date);
            if($sort_tour_start_date=='ASC'){
                $this->_pdata['sort_tour_start_date'] = 'entypo-up-open';
            }
            else{
                $this->_pdata['sort_tour_start_date'] = 'entypo-down-open';
            }
            
        } else {
            $search_conditions['sort_tour_start_date'] = null;
            $this->_pdata['sort_tour_start_date'] = null;
        }
        ...
        $this->load->model('__admin_booking/Tour_booking_mdl');
        $tour_bookings	= $this->Tour_booking_mdl->get_tour_bookings($search_conditions, $start, $limit);
       
        $customer_ids_array = array();
        $this->_pdata['tour_bookings'] = array();
        
		foreach ($tour_bookings as $tour_booking) {
		    $customer_ids_array[] = $tour_booking['booking_customer_id'];    
		    $this->_pdata['tour_bookings'][] = $tour_booking;
        }
        
        if(isset($customer_ids_array) && !empty($customer_ids_array)){
        $customers = $this->Tour_booking_mdl->get_customers_name_by_ids($customer_ids_array);
        ...
        $this->_pdata['total_tour_bookings'] = $this->Tour_booking_mdl->get_total_tour_bookings($search_conditions);

        $this->load->helper('paginator');
        $this->_pdata['pagination'] = generate_pagination($this, 'admin/booking/tour-bookings', $this->_pdata['total_tour_bookings'], $limit);
        ...
        $payment_statuses = $this->Tour_booking_mdl->get_payment_statuses();
        foreach ($payment_statuses as $payment_status) {
            $this->_pdata['payment_statuses'][$payment_status['id']] = $payment_status['name'];
        }
        
        $booking_statuses = $this->Tour_booking_mdl->get_booking_statuses();

        foreach ($booking_statuses as $booking_status) {
            $this->_pdata['booking_statuses'][$booking_status['id']] = $booking_status['name'];
        }

        $this->template_lib->load_view($this, '__admin_booking/Tour_booking_list_view', $this->_pdata);
    }

    public function get() {
        $secure_token = $this->input->get('secure_token');
        $tour_booking_id = $this->security_lib->decrypt($secure_token);

        $this->load->model('__admin_booking/Tour_booking_mdl');
        $this->_pdata['tour_booking'] = $this->Tour_booking_mdl->get_tour_booking($tour_booking_id);
        ...
        $this->template_lib->load_view($this, '__admin_booking/Tour_booking_view', $this->_pdata);
    }
    ...
    public function confirm_bookings(){
        ...
        $this->Tour_booking_mdl->confirm_bookings($this->input->get('data'));
    ...
    public function cancel_bookings(){
		...
		$this->load->model('__admin_customer/Customer_mdl');
		$booking_ids = $this->input->get('data');
        $this->Tour_booking_mdl->cancel_bookings($this->input->get('data'));
		$this->load->library('__commonlib/Email_lib');
		foreach($booking_ids as $bookingid){
			$email_data = array();
			$row_bookings = $this->Bookings_mdl->get_booking($bookingid);
