...
/*
 * APPLICATION 		: TOUR BOOKINGS CONTROLLER
 * AUTHOR			: Kundan Kumar
 * CONTRIBUTOR      : KUNDAN KUMAR, VINAY KUMAR SHARMA
 * VERSION			: 1.0
...
class Tour_bookings_ctrl extends CI_Controller {
    ...
    public function index() {
        ...
        $this->load->model('__vendor_booking/Tour_bookings_mdl');
        $tour_bookings	= $this->Tour_bookings_mdl->get_tour_bookings($search_conditions, $start, $limit);
		...
		foreach ($tour_bookings as $tour_booking) {
		    ...
		    $booking_ids_array[] = $tour_booking['booking_id'];    
		    $this->_pdata['tour_bookings'][] = $tour_booking;
        }
		
		if(isset($booking_ids_array) && !empty($booking_ids_array)) {
            $invoice_paths = $this->Tour_bookings_mdl->get_invoice_paths($booking_ids_array);
        } else {
            $invoice_paths = array();
        }
        ...
        if(isset($customer_ids_array) && !empty($customer_ids_array)) {
            $customers = $this->Tour_bookings_mdl->get_customers_name_by_ids($customer_ids_array);
        } else {
            $customers = array();
        }
        ...
        $this->_pdata['total_tour_bookings'] = $this->Tour_bookings_mdl->get_total_tour_bookings($search_conditions);

        $this->load->helper('paginator');
        $this->_pdata['pagination'] = generate_pagination($this, 'vendor/booking/tour-bookings', $this->_pdata['total_tour_bookings'], $limit);
        ...
        $payment_statuses = $this->Tour_bookings_mdl->get_payment_statuses();

        foreach ($payment_statuses as $payment_status) {
            $this->_pdata['payment_statuses'][$payment_status['id']] = $payment_status['name'];
        }

        $booking_statuses = $this->Tour_bookings_mdl->get_booking_statuses();

        foreach ($booking_statuses as $booking_status) {
            $this->_pdata['booking_statuses'][$booking_status['id']] = $booking_status['name'];
        }
		
		//pr($this->_pdata['tour_bookings']);
		
        $this->template_lib->load_view($this, '__vendor_booking/Tour_bookings_list_view', $this->_pdata);
    }
	 ...
	 public function get() {
        ...
        $this->load->model('__vendor_booking/Tour_bookings_mdl');
        $this->_pdata['tour_booking'] = $this->Tour_bookings_mdl->get_tour_booking($tour_booking_id);
        ...
        $this->template_lib->load_view($this, '__vendor_booking/Tour_bookings_view', $this->_pdata);
    }
	...
	public function invoice() {
		...
		$this->load->model('__vendor_booking/Tour_bookings_mdl');
        $invoice = $this->Tour_bookings_mdl->get_invoice_path($tour_booking_id);
	...
	public function raise_invoice() {
	    ...
	    if ($this->input->is_ajax_request()) {
	        ...
	        if ($this->form_validation->run() == FALSE) {
	            ...
	            $flag = $this->Tour_bookings_mdl->add_invoice_no_with_date($data);
	            /* 
	             * END OF FIXED
	             * 
	             *  */
