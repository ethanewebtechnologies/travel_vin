...
class Transportation_bookings_ctrl extends CI_Controller {
    ...
    public function index() {
        ...
        $this->load->model('__vendor_booking/Transportation_bookings_mdl');
        ...
        foreach ($transportation_bookings as $transportation_booking) {
			...
			$booking_ids_array[] = $transportation_booking['booking_id'];   
            $this->_pdata['transportation_bookings'][] = $transportation_booking;
        }
		
		if(isset($booking_ids_array) && !empty($booking_ids_array)) {
            $invoice_paths = $this->Transportation_bookings_mdl->get_invoice_paths($booking_ids_array);
        } else {
            $invoice_paths = array();
        }
        ...
        $this->_pdata['pagination'] = generate_pagination($this, 'vendor/booking/transportation-bookings', $this->_pdata['total_transportation_bookings'], $limit);
        ...
        $this->_pdata['text_transportation_start_date'] = $this->lang->line('text_transportation_start_date');
        $this->_pdata['text_booking_status'] = $this->lang->line('text_booking_status');
        $this->_pdata['text_payment_status'] = $this->lang->line('text_payment_status');
        ...
        $this->template_lib->load_view($this, '__vendor_booking/Transportation_bookings_list_view', $this->_pdata);
    }
    ...
    public function get() {
        ...
        $this->template_lib->load_view($this, '__vendor_booking/Transportation_bookings_view', $this->_pdata);
    }
    ...
    public function invoice() {
        ...
        $this->load->model('__vendor_booking/Transportation_bookings_mdl');
        $invoice = $this->Transportation_bookings_mdl->get_invoice_path($transportation_booking_id);
    ...
    public function raise_invoice() {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->Transportation_bookings_mdl->add_invoice_no_with_date($data);
    ...
    /* public function raise_invoice() {
        ...
        foreach ($requested_booking_ids as $requested_booking_id) {
            $this->db->where('booking_id', $requested_booking_id);
            $this->db->update('tv_transportation_bookings_tbl', array('vendor_request_status' => 1));
        }
