...
/*
 * APPLICATION 		: Golf Invoice Request Controller
 * AUTHOR			: KUNDAN KUMAR
...
class Golf_invoice_request_ctrl extends CI_Controller {
    ...
    public function index() {
        ...
        $search_golf_invoice_date = $this->input->get('search_golf_invoice_date');

        if (isset($search_golf_invoice_date)) {
            $search_conditions['search_golf_invoice_date'] = trim($search_golf_invoice_date);
            $this->_pdata['search_golf_invoice_date'] = trim($search_golf_invoice_date);
        } else {
            $search_conditions['search_golf_invoice_date'] = null;
            $this->_pdata['search_golf_invoice_date'] = null;
        }
        ...
        $sort_golf_invoice_date = $this->input->get('sort_golf_invoice_date');
        
        if (isset($sort_golf_invoice_date)) {
            $search_conditions['sort_golf_invoice_date'] = trim($sort_golf_invoice_date);
            if($sort_golf_invoice_date=='ASC'){
                $this->_pdata['sort_golf_invoice_date'] = 'entypo-up-open';
            }
            else{
                $this->_pdata['sort_golf_invoice_date'] = 'entypo-down-open';
            }
            
        } else {
            $search_conditions['sort_golf_invoice_date'] = null;
            $this->_pdata['sort_golf_invoice_date'] = null;
        }
        ...
        $this->load->model('__admin_agent/Golf_invoice_request_mdl');
       
        $golf_bookings	= $this->Golf_invoice_request_mdl->get_golf_bookings($search_conditions, $start, $limit);
        ...
        $this->_pdata['total_golf_bookings'] = $this->Golf_invoice_request_mdl->get_total_golf_bookings($search_conditions);
        ...
        $payment_statuses = $this->Golf_invoice_request_mdl->get_payment_statuses();
        
        foreach ($payment_statuses as $payment_status) {
            $this->_pdata['payment_statuses'][$payment_status['id']] = $payment_status['name'];
        }
        
        $booking_statuses = $this->Golf_invoice_request_mdl->get_booking_statuses();
 
        foreach ($booking_statuses as $booking_status) {
            $this->_pdata['booking_statuses'][$booking_status['id']] = $booking_status['name'];
        }
        
        $this->template_lib->load_view($this, '__admin_agent/Golf_invoice_request_list_view', $this->_pdata);
    }
    ...
    public function generate_invoice_in_bulk() {
        ...
        $invoice_data = $this->Golf_invoice_request_mdl->get_invoice_data_by_booking_ids($requested_booking_ids);
        
        if(isset($invoice_data[0]['agent_id'])) { 
            
            $vendor_id = $invoice_data[0]['agent_id'];
        }
        $vendor = $this->Golf_invoice_request_mdl->get_vendor_details($vendor_id);
        ...
        if(!empty($invoice_data)) {
            ...
            $countries = $this->Golf_invoice_request_mdl->get_countries();
            
            foreach ($countries as $country) {
                $invoice_template_data['countries'][$country['id']] = $country['name'];
            }
            
            $cities = $this->Golf_invoice_request_mdl->get_cities();
            ...
            $this->Golf_invoice_request_mdl->add_invoice_path_data($invoice);
        }
    ...
    public function get_cities_by_country_id() {
        ...
        $this->load->model('__admin_agent/Golf_invoice_request_mdl');
        $data['cities'] = $this->Golf_invoice_request_mdl->get_cities_by_country_id($country_id);
