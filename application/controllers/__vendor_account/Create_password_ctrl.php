...
class Create_password_ctrl extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->lang->load('__vendor_account/Create_password', $siteLang);
    ...
    public function index() {
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->Manage_mdl->create_password($this->input->post(), $this->input->post('vendor_id'));
               redirect('vendor/account/create-password/create_password_success', 'referesh');
            }
        }
        ...
        if($this->input->get('access_token')) {
            ...
            if(count($token_data) == 3) {
                ...
                $token_status = $this->Vendor_mdl->audit_token($access_token, $token);
                
                if($token_status == 1) {
                    $this->_pdata['token'] = $this->input->get('access_token');
                    
                    $this->_pdata['vendor_id']            = $token_data[0];
                    $this->_pdata['token_generated_date']   = $token_data[1];
                    $this->_pdata['vendor_email']         = $token_data[2];
                    ...
                    $this->template_lib->load_view($this, '__vendor_account/Create_password_form_view', $this->_pdata);
                    
                } else if($token_status == 2) {
                     $this->_pdata['text_token_expired'] = $this->lang->line('text_token_expire');
                    $this->template_lib->load_view($this, '__vendor_account/Change_password_denied_view', $this->_pdata);
                } else {
	...
	public function create_password_success() {
        ...
        $this->template_lib->load_view($this, '__vendor_account/Create_password_success_view', $this->_pdata);
    }
}