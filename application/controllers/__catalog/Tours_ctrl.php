...
class Tours_ctrl extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->lang->load(array('__catalog/Tours', 'form_validation'), $siteLang);
    }
    
    public function index($country_slug = null, $city_slug = null) {
        $this->load->model(array(
            '__catalog/Tours_mdl', 
            '__settings/Utility_mdl'
        ));
        ...
        $tours = $this->Tours_mdl->get_tours($search_conditions, $start, $limit);
		...
		if(!empty($tours)) {
			...
			if(isset($tour_ids_array) && !empty($tour_ids_array)) {
				
				$images = $this->Tours_mdl->get_tour_images_by_tour_ids($tour_ids_array);
			...
			$this->_pdata['total_tours'] = $this->Tours_mdl->get_total_tours($search_conditions);
			
			$this->load->helper('paginator');
			$this->_pdata['pagination'] = generate_pagination_for_front($this, 'tours', $this->_pdata['total_tours'], $limit);
		}
  
        $this->_pdata['optimized'] = $this->optimized;
        $this->_pdata['target_url'] = base_url('tours' . '/' . $this->_pdata['country_slug'] . '/' . $this->_pdata['city_slug']);
        ...
        $this->_pdata['total_category_name'] = $this->Tours_mdl->get_category_name($search_conditions['language_code']);
		...
		$configurations = $this->Utility_mdl->get_settings('tour');
		
		foreach ($configurations as $configuration) {
		    if($configuration['key'] == 'details_' . $language_code) {
		        $config = json_decode($configuration['value'], TRUE);
		        $this->_pdata['tour_configuration']['main_title'] = isset($config['main_title']) ? $config['main_title'] : '';
		        $this->_pdata['tour_configuration']['sub_title'] = isset($config['sub_title']) ? $config['sub_title'] : '';
		        $this->_pdata['tour_configuration']['boxed_title'] = isset($config['boxed_title']) ? $config['boxed_title'] : '';
		        $this->_pdata['tour_configuration']['dsc'] = isset($config['dsc']) ? $config['dsc'] : '';
		        $this->_pdata['tour_configuration']['banner_dsc'] = isset($config['banner_dsc']) ? $config['banner_dsc'] : '';
		    }
		}
		
		$tour_banners = $this->Tours_mdl->get_tour_banners();
		
		foreach ($tour_banners as $tour_banner) {
		    $this->_pdata['tour_banners'][$tour_banner['section']][] = array(
		        'image' => $tour_banner['image'],
		        'title' => $tour_banner['title']
		    );
		}
		
        $this->template_lib->load_view($this,'__catalog/Tour_list_view', $this->_pdata);
    }
	...
	public function get($country_slug = null, $city_slug = null, $tour_slug = null) {
	    ...
	    $this->template_lib->load_view($this,'__catalog/Single_tour_details', $this->_pdata);	
	}
}