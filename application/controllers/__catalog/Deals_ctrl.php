...
class Deals_ctrl extends CI_Controller {
    public function __construct() {
        ...
        $this->lang->load('__catalog/Deals', $siteLang);
    ...
    public function index($country_slug = null, $city_slug = null) {
        ...
        $this->load->model('__catalog/Tour_deals_mdl');
        $tours = $this->Tour_deals_mdl->get_tours($search_conditions, $start, $limit);
        ...
        if(!empty($tours)) {
            ...
            foreach ($tours as $key => $tour) {
                ...
                //$this->_pdata['tours'][$key]['bulk_images'] = $images;
            }
            
            $images = $this->Tour_deals_mdl->get_tour_images_by_tour_ids($tour_ids_array);
			...
			$this->_pdata['total_tours'] = $this->Tour_deals_mdl->get_total_tours($search_conditions);
        ...
        $this->_pdata['target_url'] = base_url('deals'. '/' . $this->_pdata['country_slug'] . '/' . $this->_pdata['city_slug']);
		$this->_pdata['total_category_name'] = $this->Tour_deals_mdl->get_category_name($search_conditions['language_code']);
		...
		$configurations = $this->Utility_mdl->get_settings('deals');
		
		foreach ($configurations as $configuration) {
		    if($configuration['key'] == 'details_' . $language_code) {
		        $config = json_decode($configuration['value'], TRUE);
		        $this->_pdata['deal_configuration']['main_title'] = isset($config['main_title']) ? $config['main_title'] : '';
		        $this->_pdata['deal_configuration']['sub_title'] = isset($config['sub_title']) ? $config['sub_title'] : '';
		        $this->_pdata['deal_configuration']['boxed_title'] = isset($config['boxed_title']) ? $config['boxed_title'] : '';
		        $this->_pdata['deal_configuration']['dsc'] = isset($config['dsc']) ? $config['dsc'] : '';
		        $this->_pdata['deal_configuration']['banner_dsc'] = isset($config['banner_dsc']) ? $config['banner_dsc'] : '';
		    }
		}
		...
		$tour_banners = $this->Tour_deals_mdl->get_tour_banners();
		
		foreach ($tour_banners as $tour_banner) {
		    $this->_pdata['deal_banners'][$tour_banner['section']][] = array(
		        'image' => $tour_banner['image'],
		        'title' => $tour_banner['title']
		    );
		}
		
        $this->template_lib->load_view($this,'__catalog/Tour_deals_list_view', $this->_pdata);
    }
    ...
    public function get($country_slug = null, $city_slug = null, $tour_slug = null) {
	    ...
	    $this->template_lib->load_view($this,'__catalog/Single_deal_details', $this->_pdata);	
	}
}