...
class Golfs_ctrl extends CI_Controller {
    public function __construct() {
        ...
        $this->lang->load('__catalog/Golf', $siteLang);
    ...
    public function index($country_slug = null, $city_slug = null) {
        ...
        $this->load->model('__catalog/Golf_mdl');
        ...
        if(!empty($golfs)) {
            ...
            foreach ($images as $image) {
                if(!isset($this->_pdata['golfs'][$image['item_id']]['image'])) {
            ...
            $this->_pdata['pagination'] = generate_pagination_for_front($this, 'golfs', $this->_pdata['total_golfs'][0], $limit);
		...
		$this->_pdata['target_url'] = base_url('golfs'. '/' . $this->_pdata['country_slug'] . '/' . $this->_pdata['city_slug']);
		$this->_pdata['total_category_name'] = $this->Golf_mdl->get_category_name($search_conditions['language_code']);
		...
		$configurations = $this->Utility_mdl->get_settings('golf');
		
		foreach ($configurations as $configuration) {
		    if($configuration['key'] == 'details_' . $language_code) {
		        $config = json_decode($configuration['value'], TRUE);
		        $this->_pdata['golf_configuration']['main_title'] = isset($config['main_title']) ? $config['main_title'] : '';
		        $this->_pdata['golf_configuration']['sub_title'] = isset($config['sub_title']) ? $config['sub_title'] : '';
		        $this->_pdata['golf_configuration']['boxed_title'] = isset($config['boxed_title']) ? $config['boxed_title'] : '';
		        $this->_pdata['golf_configuration']['dsc'] = isset($config['dsc']) ? $config['dsc'] : '';
		        $this->_pdata['golf_configuration']['banner_dsc'] = isset($config['banner_dsc']) ? $config['banner_dsc'] : '';
		    }
		}
		...
		$golf_banners = $this->Golf_mdl->get_golf_banners();
		
		foreach ($golf_banners as $golf_banner) {
		    $this->_pdata['golf_banners'][$golf_banner['section']][] = array(
		        'image' => $golf_banner['image'],
		        'title' => $golf_banner['title']
		    );
		}
        $this->template_lib->load_view($this,'__catalog/Golf_list_view', $this->_pdata);
    }
    
    
    
    public function get($country_slug = null, $city_slug = null, $golf_slug = null) {
        ...
        if($this->input->post()) {
            ...
            $this->form_validation->set_rules('start_date', 'lang:text_golf_start_date', 'trim|required', array(
                'required'=>$this->lang->line('required'),
            ));
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if(isset($cookie_data)) {
                    ...
                    $_cart_data_array['trip_type']['golf']['_pid'][$pid] = $_cart_data_new_array;
        ...
        if(isset($golf_slug)) {
            $search_conditions['golf_slug'] = $golf_slug;
            $this->_pdata['golf_slug'] = $golf_slug;
        } else {
            $search_conditions['golf_slug'] = null;
            $this->_pdata['golf_slug'] = null;
        }
        
        $this->load->model('__catalog/Golf_mdl');
        $this->_pdata['golf'] = $this->Golf_mdl->get_golf_detail($search_conditions);
        $this->_pdata['images'] = $this->Golf_mdl->get_golf_images_by_golf_ids($this->_pdata['golf']['golf_id']);
       
        $blockdates = $this->Golf_mdl->get_block_dates($this->_pdata['golf']['golf_id']);
        ...
        $this->session->set_userdata('country_id', $this->_pdata['golf']['country_id']);
        
		$this->_pdata['text_next']               		= $this->lang->line('text_next');
		$this->_pdata['text_previous']               	= $this->lang->line('text_previous');
        $this->_pdata['text_booking_adults'] = $this->lang->line('text_booking_adults');
        $this->_pdata['text_price'] = $this->lang->line('text_price');
        $this->_pdata['text_golf_start_date'] = $this->lang->line('text_golf_start_date');
		...
		$this->breadcrumbs->push('Home', '/');
		$this->breadcrumbs->push($class_title, $class_link.'/'.$this->_pdata['country_slug'].'/'.$this->_pdata['city_slug']);
		$this->breadcrumbs->push($this->_pdata['golf']['title'], $class_link.'/'.$this->_pdata['country_slug'].'/'.$this->_pdata['city_slug'].'/'.$this->_pdata['golf_slug']);
		// output
		$this->_pdata['output_breadcrumb'] = $this->breadcrumbs->show();
        
        $this->template_lib->load_view($this,'__catalog/Single_golf_details', $this->_pdata);
    }
}