<section class="search-results-env">
    <div class="row">
    	<div class="col-md-12">
            <h2>
            	Golf Booking Invoice Request Here ...
    		</h2>
    		<hr>
            ...
            <div class="row">	
				...
				<?php
					$attributes = array(
						'name' => 'search_golf_invoice_date',
						'id' => 'search_golf_invoice_date',
						'placeholder' => 'Search for date   ...',
						'class' => 'form-control input-lg datepicker',
						'value' => $search_golf_invoice_date
					);
					echo form_input($attributes);
				?>
            	</div>
        		...
        		<table class="table table-bordered table-striped datatable" id="table-2">
                	<thead>
                		<tr>
                			...
                			<th class="col-sm-2">
                			Invoice Date <span class="pull-right"><i class="<?php if(isset($sort_golf_invoice_date)){echo $sort_golf_invoice_date;}else{echo 'entypo-up-open';}?>" id="sort-by-golf-invoice-date"></i></span>
                			</th>
                			<th class="col-sm-2">
						...
						<?php if($golf_bookings) { ?>
    						<?php $sr = 1; ?>
    							<?php foreach($golf_bookings as $golf_booking) { ?>
    								<tr>
    									<td>
                            				<?php if(isset($golf_booking['invoice_path']) && !empty($golf_booking['invoice_path'])) { ?>
                            				<?php } else { ?>
                                				<div class="checkbox checkbox-replace">
                        				...
                        				<td>
                        					<?php echo isset($golf_booking['vendor_invoice_no']) ? $golf_booking['vendor_invoice_no'] : THREE_DASH; ?>
                        				</td>
                        				
                        				<td>
                        					<?php echo d_to_lu($golf_booking['date_generated']); ?>
                        				</td>
                        				<td>
                        					<?php echo isset($golf_booking['company_legal_name']) ? $golf_booking['company_legal_name'] : THREE_DASH; ?>
                        				</td>
										<td>
											<div class="label <?php if(isset($booking_statuses[$golf_booking['booking_status']]) && $booking_statuses[$golf_booking['booking_status']]=='Completed'){echo 'label-success';}else{echo 'label-danger';}?>">
                    					...
                    					<td>
                    						<a href="<?php echo base_url('admin/booking/golf-bookings/get?secure_token=' . $this->security_lib->encrypt($golf_booking['booking_id']).'&back_url=agent/invoice-request'); ?>" class="btn btn-blue btn-sm btn-icon icon-left">
...
<script>
$(function() {
    $('#search_golf_invoice_date').datepicker({
		format: 'dd-mm-yyyy',     
    });
});
</script>
...
$("#sort-by-golf-invoice-date").click(function(){
	var attr_class = $(this).attr('class');
	var sort_key;
	if(attr_class=='entypo-up-open'){$(this).attr('class','entypo-down-open');sort_key = 'DESC';}
	else if(attr_class=='entypo-down-open'){$(this).attr('class','entypo-up-open');sort_key = 'ASC';}
	else{$(this).attr('class','entypo-up-open');sort_key = 'ASC';}
	url = url+'?sort_golf_invoice_date='+sort_key;
	window.location.href = url;
});
</script>
...
function generateInvoiceInBulk() {
    ...
    if(requestedBookingIds.length > 0) {
        ...
        var post_url = "<?php echo base_url('admin/agent/golf-invoice-request/generate_invoice_in_bulk'); ?>";
