<section class="search-results-env">
    <div class="row">
    	<div class="col-md-12">
            <h2>
            	Transportation Booking Invoice Request Here ...
        	...
        	<?php
                ...
                echo form_open('admin/agent/transportation-invoice-request', $attributes);
    		?>
			<!-- CSRF NAME -->
			<?php
				$attributes = array(
            ...
            <div class="row">	
				...
				<?php
					$attributes = array(
						'name' => 'search_transportation_invoice_date',
						'id' => 'search_transportation_invoice_date',
						'placeholder' => 'Search for date   ...',
						'class' => 'form-control input-lg datepicker',
						'value' => $search_transportation_invoice_date
					);
					echo form_input($attributes);
				?>
            	</div>
        		...
        		<table class="table table-bordered table-striped datatable" id="table-2">
                	<thead>
                		<tr>
                			...
                			<th class="col-sm-2">
                			Invoice Date <span class="pull-right"><i class="<?php if(isset($sort_transportation_invoice_date)){echo $sort_transportation_invoice_date;}else{echo 'entypo-up-open';}?>" id="sort-by-transportation-invoice-date"></i></span>
                			</th>
                			<th class="col-sm-2">
						...
						<?php if($transportation_bookings) { ?>
    						<?php $sr = 1; ?>
    							<?php foreach($transportation_bookings as $transportation_booking) { ?>
    								<tr>
    									<td>
                            				<?php if(isset($transportation_booking['invoice_path']) && !empty($transportation_booking['invoice_path'])) { ?>
                            				<?php } else { ?>
                                				<div class="checkbox checkbox-replace">
                                					<input type="checkbox" id="checked-ids-<?php echo $transportation_booking['booking_id']; ?>" name="checked_ids[]" value="<?php echo $transportation_booking['booking_id']; ?>">
                                				</div>
                            				<?php } ?>
                            			</td>
                        				<td>
                        				...
                        				<td>
                        					<?php echo isset($transportation_booking['vendor_invoice_no']) ? $transportation_booking['vendor_invoice_no'] : THREE_DASH; ?>
                        				</td>
                        				
                        				<td>
                        					<?php echo d_to_lu($transportation_booking['date_generated']); ?>
                        				</td>
                        				<td>
                        					<?php echo isset($transportation_booking['company_legal_name']) ? $transportation_booking['company_legal_name'] : THREE_DASH; ?>
                        				</td>
										<td>
											<div class="label <?php if(isset($booking_statuses[$transportation_booking['booking_status']]) && $booking_statuses[$transportation_booking['booking_status']]=='Completed'){echo 'label-success';}else{echo 'label-danger';}?>">
                    					...
                    					<td>
                    						<a href="<?php echo base_url('admin/booking/transportation-bookings/get?secure_token=' . $this->security_lib->encrypt($transportation_booking['booking_id']).'&back_url=agent/transportation-invoice-request'); ?>" class="btn btn-blue btn-sm btn-icon icon-left">
...
<script>
$(function() {
    $('#search_transportation_invoice_date').datepicker({
		format: 'dd-mm-yyyy',     
    });
});
</script>
...
$("#sort-by-transportation-invoice-date").click(function(){
	var attr_class = $(this).attr('class');
	var sort_key;
	if(attr_class=='entypo-up-open'){$(this).attr('class','entypo-down-open');sort_key = 'DESC';}
	else if(attr_class=='entypo-down-open'){$(this).attr('class','entypo-up-open');sort_key = 'ASC';}
	else{$(this).attr('class','entypo-up-open');sort_key = 'ASC';}
	url = url+'?sort_transportation_invoice_date='+sort_key;
	window.location.href = url;
});
</script>
...
function generateInvoiceInBulk() {
    ...
    if(requestedBookingIds.length > 0) {
        ...
        var post_url = "<?php echo base_url('admin/agent/transportation-invoice-request/generate_invoice_in_bulk'); ?>";
