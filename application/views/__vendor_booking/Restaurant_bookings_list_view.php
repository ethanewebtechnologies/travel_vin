    ...
    <div class="row">
    	<div class="col-md-12">
            ...
            <?php
                ...
                echo form_open('vendor/booking/restaurant-bookings', $attributes);
            ?>
                <!-- CSRF NAME -->
                <?php
                    $attributes = array(
					...
					<tbody>
						<?php if($restaurant_bookings) { ?>
    						<?php foreach($restaurant_bookings as $restaurant_booking) { ?>
								<tr>
                        			<td>
                        				<?php if(isset($invoice_paths[$restaurant_booking['booking_id']]) && !empty($invoice_paths[$restaurant_booking['booking_id']])) { ?>
                        				<?php } else { ?>
                            				<div class="checkbox checkbox-replace">
                    				...
                    				<td>
                    					<?php if(isset($invoice_paths[$restaurant_booking['booking_id']]) && !empty($invoice_paths[$restaurant_booking['booking_id']])) { ?> 
											<?php $href =  base_url('vendor/booking/restaurant-bookings/invoice?secure_token=' . $this->security_lib->encrypt($restaurant_booking['booking_id'])); ?>
										<?php } else { ?>
									...
									<td>
										<p>
											<small>
												<b>Customer: </b> <?php echo isset($customers[$restaurant_booking['booking_customer_id']]) ? $customers[$restaurant_booking['booking_customer_id']] : THREE_DASH; ?>
											</small>
										</p>
										<p>
											<small>
												<b>Start Date: </b> <?php echo d_to_lu($restaurant_booking['start_date']); ?> 
											</small>
										</p>
										<!--<p>
											<small>
												<b>Duration: </b> <?php echo duration($restaurant_booking['restaurant_start_date'], $restaurant_booking['restaurant_end_date']); ?>
											</small>
										</p>-->
							        </td>
                					...
                					<td>
            							<a href="<?php echo base_url('vendor/booking/restaurant-bookings/get?secure_token=' . $this->security_lib->encrypt($restaurant_booking['booking_id'])); ?>" class="btn btn-blue btn-sm btn-icon icon-left">
                    						<i class="entypo-eye" ></i> <?php echo $text_view; ?>
                    				 	</a>
                						<?php if(isset($invoice_paths[$restaurant_booking['booking_id']]) && !empty($invoice_paths[$restaurant_booking['booking_id']])) { ?>
											<a href="<?php echo base_url('vendor/booking/restaurant-bookings/download?secure_token=' . $this->security_lib->encrypt($restaurant_booking['booking_id'])); ?>" class="btn btn-green btn-sm btn-icon icon-left">
			...
			<div class="modal-content">
    			...
    			<div class="modal-footer">				       	
                    <span class="pull-right btn-toolbar">
                    	...
                    	<a class="btn btn-default" href="<?php echo base_url('vendor/booking/restaurant-bookings'); ?>">
                			Close
                		</a>
    				</span>
    			</div>
...
$(document).ready(function() {
	$('#save_changes').click(function(e) {
		...
		var post_url = "<?php echo base_url('vendor/booking/restaurant-bookings/raise-invoice'); ?>";
            ...
            data[secure_token] = secure_hash;
    		 ...
    		 $.ajax({
        		...
        		success: function (response) {
        	    	...
        	    	window.location.href="<?php echo base_url('vendor/booking/restaurant-bookings'); ?>";
        		}
        	});
    	} else {
        	var opts = {
