    ...
    <div class="row">
    	<div class="col-md-12">
            ...
            <?php
                ...
                echo form_open('vendor/booking/tour-bookings', $attributes);
            ?>
                <!-- CSRF NAME -->
                <?php
                    $attributes = array(
        		...
        		<div class="input-group">
    				<?php 
                        $attributes = array(
                            ...
                            'placeholder' => 'Search Tour Bookings no...',
					...
					<tbody>
						<?php if($tour_bookings) { ?>
							...
							<?php foreach($tour_bookings as $tour_booking) { ?>
								<tr>
                        			<td>
                        				<?php if(isset($invoice_paths[$tour_booking['booking_id']]) && !empty($invoice_paths[$tour_booking['booking_id']])) { ?>
                        				<?php } else { ?>
                            				<div class="checkbox checkbox-replace">
                            					<input type="checkbox" id="checked-ids-<?php echo $tour_booking['booking_id']; ?>" name="checked_ids[]" value="<?php echo $tour_booking['booking_id']; ?>">
                            				</div>
                        				<?php } ?>
                        			</td>
                    				...
                    				<td>
                    					<?php if(isset($invoice_paths[$tour_booking['booking_id']]) && !empty($invoice_paths[$tour_booking['booking_id']])) { ?> 
											<?php $href =  base_url('vendor/booking/tour-bookings/invoice?secure_token=' . $this->security_lib->encrypt($tour_booking['booking_id'])); ?>
										<?php } else { ?>
									...
									<td>
										<p>
											<small>
												<b>Customer: </b> <?php echo isset($customers[$tour_booking['booking_customer_id']]) ? $customers[$tour_booking['booking_customer_id']] : THREE_DASH; ?>
											</small>
										</p>
										<p>
											<small>
												<b>Start Date: </b> <?php echo d_to_lu($tour_booking['tour_start_date']); ?> 
											</small>
										</p>
										<p>
											<small>
												<b>Duration: </b> <?php echo duration($tour_booking['tour_start_date'], $tour_booking['tour_end_date']); ?>
											</small>
										</p>
							        </td>
                					...
                					<td>
            							<a href="<?php echo base_url('vendor/booking/tour-bookings/get?secure_token=' . $this->security_lib->encrypt($tour_booking['booking_id'])); ?>" class="btn btn-blue btn-sm btn-icon icon-left">
                    						<i class="entypo-eye" ></i> <?php echo $text_view; ?>
                    				 	</a>
                						<?php if(isset($invoice_paths[$tour_booking['booking_id']]) && !empty($invoice_paths[$tour_booking['booking_id']])) { ?>
											<a href="<?php echo base_url('vendor/booking/tour-bookings/download?secure_token=' . $this->security_lib->encrypt($tour_booking['booking_id'])); ?>" class="btn btn-green btn-sm btn-icon icon-left">
			...
			<div class="modal-content">
    			...
    			<div class="modal-footer">				       	
                    <span class="pull-right btn-toolbar">
                    	...
                    	<a class="btn btn-default" href="<?php echo base_url('vendor/booking/tour-bookings'); ?>">
                			Close
                		</a>
    				</span>
    			</div>
...
$(document).ready(function() {
	$('#save_changes').click(function(e) {
		...
		var post_url = "<?php echo base_url('vendor/booking/tour-bookings/raise-invoice'); ?>";
            ...
            data[secure_token] = secure_hash;
    		 ...
    		 $.ajax({
        		...
        		success: function (response) {
        	    	...
        	    	window.location.href="<?php echo base_url('vendor/booking/tour-bookings'); ?>";
        		}
        	});
    	} else {
        	var opts = {
