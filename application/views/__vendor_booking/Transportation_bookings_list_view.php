    ...
    <div class="row">
    	<div class="col-md-12">
            ...
            <?php
                ...
                echo form_open('vendor/booking/transportation-bookings', $attributes);
            ?>
                <!-- CSRF NAME -->
                <?php
                    $attributes = array(
        		...
        		<div class="input-group">
    				<?php 
                        $attributes = array(
                            ...
                            'placeholder' => 'Search transportation Bookings...',
					...
					<tbody>
						<?php if($transportation_bookings) { ?>
							...
							<?php foreach($transportation_bookings as $transportation_booking) { ?>
								<tr>
                        			<td>
                        				<?php if(isset($invoice_paths[$transportation_booking['booking_id']]) && !empty($invoice_paths[$transportation_booking['booking_id']])) { ?>
                        				<?php } else { ?>
                            				<div class="checkbox checkbox-replace">
                    				...
                    				<td>
                    					<?php if(isset($invoice_paths[$transportation_booking['booking_id']]) && !empty($invoice_paths[$transportation_booking['booking_id']])) { ?> 
											<?php $href =  base_url('vendor/booking/transportation-bookings/invoice?secure_token=' . $this->security_lib->encrypt($transportation_booking['booking_id'])); ?>
										<?php } else { ?>
                					...
                					<td>
            							<a href="<?php echo base_url('vendor/booking/transportation-bookings/get?secure_token=' . $this->security_lib->encrypt($transportation_booking['booking_id'])); ?>" class="btn btn-blue btn-sm btn-icon icon-left">
                    						<i class="entypo-eye" ></i> <?php echo $text_view; ?>
                    				 	</a>
                						<?php if(isset($invoice_paths[$transportation_booking['booking_id']]) && !empty($invoice_paths[$transportation_booking['booking_id']])) { ?>
											<a href="<?php echo base_url('vendor/booking/transportation-bookings/download?secure_token=' . $this->security_lib->encrypt($transportation_booking['booking_id'])); ?>" class="btn btn-green btn-sm btn-icon icon-left">
			...
			<div class="modal-content">
    			...
    			<div class="modal-footer">				       	
                    <span class="pull-right btn-toolbar">
                    	...
                    	<a class="btn btn-default" href="<?php echo base_url('vendor/booking/transportation-bookings'); ?>">
                			Close
                		</a>
    				</span>
    			</div>
...
$(document).ready(function() {
	$('#save_changes').click(function(e) {
		...
		var post_url = "<?php echo base_url('vendor/booking/transportation-bookings/raise-invoice'); ?>";
            ...
            data[secure_token] = secure_hash;
    		 ...
    		 $.ajax({
        		...
        		success: function (response) {
        	    	...
        	    	window.location.href="<?php echo base_url('vendor/booking/transportation-bookings'); ?>";
        		}
        	});
    	} else {
        	var opts = {
