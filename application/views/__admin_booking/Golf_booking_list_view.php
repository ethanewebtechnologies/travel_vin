    ...
    <div class="row">
    	<div class="col-md-12">
        	...
        	<?php
                ...
                echo form_open('admin/booking/golf-bookings', $attributes);
    		?>
    		  <div class="row">
                	<div class="col-sm-5">
                		<?php
            ...
            <br>
             <div class="row">	
            	...
            	<div class="col-sm-5">
            		<?php
                        $attributes = array(
                            'name' => 'search_golf_start_date',
                            'id' => 'search_golf_start_date',
                            'placeholder' => 'Search for date   ...',
                            'autocomplete' => 'off',
                            'class' => 'form-control input-lg datepicker',
                            'value' => $search_golf_start_date
                        );
        		...
        		<table class="table table-bordered table-striped datatable" id="table-2">
                	<thead>
                		<tr>
                			...
                			<th class="col-sm-2">
                				Golf Start Date <span class="pull-right"><i class="<?php if(isset($sort_golf_start_date)){echo $sort_golf_start_date;}else{echo 'entypo-up-open';}?>" id="sort-by-date"></i></span>
                			</th>
    						...
    						<?php $sr = 1; ?>
    							<?php foreach($golf_bookings as $golf_booking) { ?>
    								<tr>
                        				<td>
    										<div class="checkbox checkbox-replace">
                                				<input type="checkbox" name="checked_ids[]" value="<?php echo $golf_booking['booking_id']; ?>">
                                			</div>
    									</td>
    									<td>
    										<small>
                        				...
                        				<td>
                        					<?php echo isset($customers[$golf_booking['booking_customer_id']]) ? $customers[$golf_booking['booking_customer_id']] : THREE_DASH; ?>
                        				</td>
                        					<td>
                        				<?php echo $golf_booking['company_legal_name']; ?>
                        				</td>
                        				<td>
                        					<?php if($golf_booking['start_date'] != '0000-00-00 00:00:00') { ?>
                        				...
                        				<td>
                        				<div class="label <?php if($payment_statuses[$golf_booking['payment_status']]=='Completed'){
											                             echo 'label-success';
                        				                            }elseif($payment_statuses[$golf_booking['payment_status']]=='Pending'){
                    											            echo 'label-warning ';
                        			                             	}elseif($payment_statuses[$golf_booking['payment_status']]=='Failed'){
                    											          echo 'label-warning ';
                        			                             	}elseif($payment_statuses[$golf_booking['payment_status']]=='Initialize') {
                    											           echo 'label-info'; 
                    										          }
                    										         else {
                    											          echo 'label-info';
                    					...
                    					<td>
                    						<a href="<?php echo base_url('admin/booking/golf-bookings/get?secure_token=' . $this->security_lib->encrypt($golf_booking['booking_id'])); ?>" class="btn btn-blue btn-sm btn-icon icon-left">
                        						<i class="entypo-eye" ></i> <?php echo $text_view; ?>
                        				 	</a>
                        					<!--<a href="<?php echo base_url('admin/booking/golf-bookings/edit?secure_token=' . $this->security_lib->encrypt($golf_booking['booking_id'])); ?>" class="btn btn-default btn-sm btn-icon icon-left">
...
<script>
$(function() {
    $('#search_golf_start_date').datepicker({
		format: 'dd-mm-yyyy',     
    });
});
</script>
...
$("#sort-by-booking-no").click(function(){
	...
	window.location.href = '<?php echo base_url()?>admin/booking/golf-bookings?sort_booking_no='+sort_key;
});
$("#sort-by-customer-name").click(function(){
	...
	window.location.href = '<?php echo base_url()?>admin/booking/golf-bookings?sort_customer_name='+sort_key;
});
$("#sort-by-agent-name").click(function(){
	...
	window.location.href = '<?php echo base_url()?>admin/booking/golf-bookings?sort_agent_name='+sort_key;
});
...
$("#sort-by-date").click(function(){
	...
	window.location.href = '<?php echo base_url()?>admin/booking/golf-bookings?sort_golf_start_date='+sort_key;
});
</script>
...
function confirmClick() {
    ...
    if(requestedBookingIds.length > 0) {
    	 $.ajax({
    		 	url: '<?php echo base_url()?>admin/booking/golf-bookings/confirm_bookings',
   			method: 'get',
     		data: {data:requestedBookingIds}, 
     		dataType: 'html',
     		success: function (response) {
     			window.location.href="<?php echo base_url()?>admin/booking/golf-bookings";
     		}
     	});
    } else {
...
function cancelClick() {
    ...
    if(requestedBookingIds.length > 0) {
    	 $.ajax({
    		 	url: '<?php echo base_url()?>admin/booking/golf-bookings/cancel_bookings',
   			method: 'get',
     		data: {data:requestedBookingIds}, 
     		dataType: 'html',
     		success: function (response) {
