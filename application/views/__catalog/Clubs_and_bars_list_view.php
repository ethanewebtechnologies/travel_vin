<main>
								...
								<div class="row">
									...
									<div class="col-sm-3">
										...
										<div class="countries">
											...
											<a class="dropdown-item" href="<?php echo base_url('clubs-and-bars/' . $value['seo_url'] . '/' . $data['seo_url']); ?>"><?php echo $data['name'];?></a>
											<?php }}?>
										</div>
									</div>
									<?php }?>
	...
	<?php if(isset($club_and_bar_banners['main']) && !empty($club_and_bar_banners['main'])){?>
				...
				<div class="banner">
					<div id="carouselExampleControls" class="carousel slide" data-ride="carousel">
						<div class="carousel-inner">
							<?php 
							foreach ($club_and_bar_banners['main'] as $key => $club_and_bar_banner) { ?>
							<?php if(isset($club_and_bar_banner['image']) && file_exists($club_and_bar_banner['image'])) { ?>
								<div class="item <?php echo $key == 0 ? 'active' : ''; ?>">
        								<?php echo $optimized->resize($club_and_bar_banner['image'], 1349, 521, array('class' => 'd-block w-100 img-fluid', 'alt' => $club_and_bar_banner['title']), 'resize_crop'); ?>
								</div>
    							<?php } ?>
							<?php } ?>
						</div>
					...
					<div class="bnr_text">
						<div class="bnr_wrap_text">
							<h1 class="wow zoomIn" data-wow-delay="1s"><?php echo $club_and_bar_configuration['main_title']; ?></h1>
							<h5><?php echo $club_and_bar_configuration['sub_title']; ?></h5>
							<div>
								<p class="spcl_txt"><?php echo $club_and_bar_configuration['boxed_title']; ?></p>
							</div>
						</div>
					</div>
				</div>
	...
	<?php } ?>
	<?php if(isset($club_and_bar_configuration['banner_dsc']) && !empty($club_and_bar_configuration['banner_dsc'])){?>
	<section class="transport_blk">
		<section class="container">
			<section class="row">
				<div class="col-sm-12">						
					<div class="form_text_blk no-shadow">
						<h3><?php echo $club_and_bar_configuration['banner_dsc']; ?></h3>
					</div>
				</div>
			</section>
		</section>
							...
							<div class="filter_search_blk">
									...
									<?php if ($search_title) { ?>
										<h2><?php echo $text_Search;?></h2>
								   				<?php if (isset($total_club_and_bars)) { ?>
                                					<?php if ($total_club_and_bars > 1) { ?>
                                   	 				<?php echo $total_club_and_bars; ?> <?php echo $text_results;?>
                               						<?php } else if ($total_club_and_bars == 1) { ?>
                                  							  <?php echo $total_club_and_bars; ?> <?php echo $text_result;?>
                                							<?php } ?>
                            					<?php } else { ?>
					...
					<div class="listing_blk wow pulse" data-wow-delay="0.8s" style="visibility: visible; animation-delay: 0.8s; animation-name: pulse;">
						<?php if(!empty($club_and_bars)) { ?>
							...
							<div class="tours_list_wrap">
								<?php foreach ($club_and_bars as $club_and_bar) { ?>
									<div class="single_list_blk">
        								<div class="tourimg">
        									<?php if(isset($club_and_bar['image']) && file_exists($club_and_bar['image'])) { ?>
        										<?php 
        										      echo $optimized->resize($club_and_bar['image'], 230, 153, array(), 'resize_crop');
        									    ?>
    										<?php } else { ?>
    									...
    									<div class="tour_short_detail">
        									...
        									<p>
        										<!-- overview -->
        										<?php 
                                                    if(!empty($club_and_bar['dsc'])) { 
                                                        // Strip tags to avoid breaking any html
        										        $club_and_bar_dsc = strip_tags($club_and_bar['dsc']);
            									        
            											// truncate string
        										        $short_club_and_bar_dsc = substr($club_and_bar_dsc, 0, 120);
        										        echo $short_club_and_bar_dsc . THREE_DOTS;
            										} else {
            										    echo THREE_DASH;
            										}
            									?>
        								...
        								<div class="price_detail">
        									<h4><?php echo $text_from_usd;?></h4>
            									<h2> $<?php echo number_format($club_and_bar['price'], PRICE_DELIMITER); ?></h2>           							
        									<a href="<?php echo base_url('clubs-and-bars/' . $club_and_bar['country_seo_url'] . '/' . $club_and_bar['city_seo_url'] . '/' . $club_and_bar['slug']); ?>"><?php echo $text_Book_Now;?></a>
        								</div>
    								</div>
								<?php } ?>
