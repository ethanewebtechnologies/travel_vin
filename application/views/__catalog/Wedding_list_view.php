<main>
								...
								<div class="row">
									...
									<div class="col-sm-3">
										...
										<div class="countries">
											...
											<a class="dropdown-item" href="<?php echo base_url('weddings/' . $value['seo_url'] . '/' . $data['seo_url']); ?>"><?php echo $data['name'];?></a>
											<?php }}?>
										</div>
									</div>
									<?php }?>
	...
	<?php if(isset($wedding_banners['main']) && !empty($wedding_banners['main'])){?>
				...
				<div class="banner">
					<div id="carouselExampleControls" class="carousel slide" data-ride="carousel">
						<div class="carousel-inner">
							<?php 
							foreach ($wedding_banners['main'] as $key => $wedding_banner) { ?>
								<?php if(isset($wedding_banner['image']) && file_exists($wedding_banner['image'])) { ?>
								<div class="item <?php echo $key == 0 ? 'active' : ''; ?>">
        								<?php echo $optimized->resize($wedding_banner['image'], 1349, 521, array('class' => 'd-block w-100 img-fluid', 'alt' => $wedding_banner['title']), 'resize_crop'); ?>
								</div>
    							<?php } ?>
							<?php }?>
						</div>
					...
					<div class="bnr_text">
						<div class="bnr_wrap_text">
							<h1 class="wow zoomIn" data-wow-delay="1s"><?php echo $wedding_configuration['main_title']; ?></h1>
							<h5><?php echo $wedding_configuration['sub_title']; ?></h5>
							<div>
								<p class="spcl_txt"><?php echo $wedding_configuration['boxed_title']; ?></p>
							</div>
						</div>
					</div>
				</div>
	...
	<?php }?>
	<?php if(isset($wedding_configuration['banner_dsc']) && !empty($wedding_configuration['banner_dsc'])){?>
	<section class="transport_blk">
		<section class="container">
			<section class="row">
				<div class="col-sm-12">						
					<div class="form_text_blk no-shadow">
						<h3><?php echo $wedding_configuration['banner_dsc']; ?></h3>
					</div>
				</div>
			</section>
		</section>
							...
							<div class="filter_search_blk">
									...
									<?php if ($search_title) { ?>
										<h2><?php echo $text_Search;?></h2>
								   				<?php if (isset($total_weddings)) { ?>
                                					<?php if ($total_weddings > 1) { ?>
                                   	 				<?php echo $total_weddings; ?> <?php echo $text_results;?>
                               						<?php } else if ($total_weddings == 1) { ?>
                                  							  <?php echo $total_weddings; ?> <?php echo $text_result;?>
                                							<?php } ?>
                            					<?php } else { ?>
									...
									<div class="single_list_blk">
        								<div class="tourimg">
        									<?php if(isset($wedding['image']) && file_exists($wedding['image'])) { ?>
        										<?php 
        										      echo $optimized->resize($wedding['image'], 230, 153, array(), 'resize_crop');
        									    ?>
    										<?php } else { ?>
    									...
    									<div class="tour_short_detail">
        									...
        									<p>
        										<!-- overview -->
        										<?php 
                                                    if(!empty($wedding['dsc'])) { 
                                                        // Strip tags to avoid breaking any html
        										        $wedding_dsc = strip_tags($wedding['dsc']);
            									        
            											// truncate string
        										        $short_wedding_dsc = substr($wedding_dsc, 0, 120);
        										        echo $short_wedding_dsc . THREE_DOTS;
            										} else {
            										    echo THREE_DASH;
            										}
            									?>
        								...
        								<div class="price_detail">
        									...
        									<a href="<?php echo base_url('weddings/' . $wedding['country_seo_url'] . '/' . $wedding['city_seo_url'] . '/' . $wedding['slug']); ?>"><?php echo $text_visit;?></a>
        								</div>
    								</div>
								<?php } ?>
