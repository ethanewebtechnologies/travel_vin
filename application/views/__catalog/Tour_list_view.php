<main>
								...
								<div class="row">
									...
									<div class="col-sm-3">
										...
										<div class="countries">
											...
											<a class="dropdown-item" href="<?php echo base_url('tours/' . $value['seo_url'] . '/' . $data['seo_url']); ?>" style="color:#337ab7;font-size: 12px;font-weight: 500;"><?php echo $data['name'];?></a>
											<?php }}?>
										</div>
									</div>
									<?php }?>
	...
	<?php if(isset($tour_banners['main']) && !empty($tour_banners['main'])){?>
				...
				<div class="banner">
					<div id="carouselExampleControls" class="carousel slide" data-ride="carousel">
						<div class="carousel-inner">
							<?php 
							foreach ($tour_banners['main'] as $key => $tour_banner) { ?>
							<?php if(isset($tour_banner['image']) && file_exists($tour_banner['image'])) { ?>
								<div class="item <?php echo $key == 0 ? 'active' : ''; ?>">
        								<?php echo $optimized->resize($tour_banner['image'], 1349, 521, array('class' => 'd-block w-100 img-fluid', 'alt' => $tour_banner['title']), 'resize_crop'); ?>
								</div>
    							<?php } ?>
							<?php } ?>
						</div>
					...
					<div class="bnr_text">
						<div class="bnr_wrap_text">
							<h1 class="wow zoomIn" data-wow-delay="1s"><?php echo $tour_configuration['main_title']; ?></h1>
							<h5><?php echo $tour_configuration['sub_title']; ?></h5>
							<div>
								<p class="spcl_txt"><?php echo $tour_configuration['boxed_title']; ?></p>
							</div>
						</div>
					</div>
				</div>
	...
	<?php } ?>
	<?php if(isset($tour_configuration['banner_dsc']) && !empty($tour_configuration['banner_dsc'])){?>
	<section class="transport_blk">
		<section class="container">
			<section class="row">
				<div class="col-sm-12">						
					<div class="form_text_blk no-shadow">
						<h3><?php echo $tour_configuration['banner_dsc']; ?></h3>
					</div>
				</div>
			</section>
		</section>
					...
					<div class="filter_blk wow pulse" data-wow-delay="0.7s" style="visibility: visible; animation-delay: 0.7s; animation-name: pulse;">
						<div class="tour_dest_blk">
							...
							<div class="tour_dest">
										...
										<div class="row">
											...
											<div class="col-sm-3">
												...
												<div class="countries">
													<?php foreach ($city_name as $data){?>
													<?php if($data['country_id']==$value['id']){?>
													<a class="dropdown-item" href="<?php echo base_url('tours/' . $value['seo_url'] . '/' . $data['seo_url']); ?>"><?php echo $data['name'];?></a>
													<?php }}?>
												</div>
											</div>
											<?php }?>
								...
								<!--<p><i class="fa fa-map-marker"></i> <?php echo isset($tours[0]['country_name']) ? $tours[0]['country_name'] : THREE_DASH; ?> &gt; <?php echo isset($tours[0]['city_name']) ? $tours[0]['city_name'] : THREE_DASH; ?></p>-->
							</div>
						</div>
							...
							<div class="filter_search_blk">
									...
									<?php if ($search_title) { ?>
										<h2><?php echo $text_Search;?></h2>
								   				<?php if (isset($total_tours)) { ?>
                                					<?php if ($total_tours > 1) { ?>
                                   	 				<?php echo $total_tours; ?> <?php echo $text_results;?>
                               						<?php } else if ($total_tours == 1) { ?>
                                  							  <?php echo $total_tours; ?> <?php echo $text_result;?>
                                							<?php } ?>
                            					<?php } else { ?>
							...
							<?php foreach ($tours as $tour) { ?>
								<div class="single_list_blk">
									...
									<?php if(isset($tour['image']) && file_exists($tour['image'])) { ?>
											<?php 
											echo $optimized->resize($tour['image'], 230, 153, array(), 'resize_crop');
											?>
										<?php } else { ?>
										...
										<p>
											...
											<?php 
												if(!empty($tour['dsc'])) { 
													...
													$tour_dsc = strip_tags($tour['dsc']);
													
													// truncate string
													$short_tour_dsc = substr($tour_dsc, 0, 120);
													echo $short_tour_dsc . THREE_DOTS;
												} else {
													echo THREE_DASH;
												}
											?>
									...
									<div class="price_detail">
										...
										<?php if(isset($tour['adult_deal_price']) && $tour['adult_deal_price']>'0') { ?>
											<strike>$ <?php echo number_format($tour['adult_price'], PRICE_DELIMITER); ?></strike>
											<h2>$ <?php echo number_format($tour['adult_deal_price'], PRICE_DELIMITER); ?></h2>
										<?php } else { ?>	
											<h2>$ <?php echo number_format($tour['adult_price'], PRICE_DELIMITER); ?></h2>
										<?php } ?>
										<a href="<?php echo base_url('tours/' . $tour['country_seo_url'] . '/' . $tour['city_seo_url'] . '/' . $tour['slug']); ?>"><?php echo $text_Book_Now;?></a>
									</div>
								</div>
							<?php } ?>
