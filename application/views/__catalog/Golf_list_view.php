<main>
								...
								<div class="row">
									...
									<div class="col-sm-3">
										...
										<div class="countries">
											...
											<a class="dropdown-item" href="<?php echo base_url('golfs/' . $value['seo_url'] . '/' . $data['seo_url']); ?>"><?php echo $data['name'];?></a>
											<?php }}?>
										</div>
									</div>
									<?php }?>
	...
	<?php if(isset($golf_banners['main']) && !empty($golf_banners['main'])){?>
				...
				<div class="banner">
					<div id="carouselExampleControls" class="carousel slide" data-ride="carousel">
						<div class="carousel-inner">
							<?php 
							foreach ($golf_banners['main'] as $key => $golf_banner) { ?>
							<?php if(isset($golf_banner['image']) && file_exists($golf_banner['image'])) { ?>
								<div class="item <?php echo $key == 0 ? 'active' : ''; ?>">
        								<?php echo $optimized->resize($golf_banner['image'], 1349, 521, array('class' => 'd-block w-100 img-fluid', 'alt' => $golf_banner['title']), 'resize_crop'); ?>
								</div>
    								<?php } ?>
							<?php } ?>
						</div>
					...
					<div class="bnr_text">
						<div class="bnr_wrap_text">
							<h1 class="wow zoomIn" data-wow-delay="1s"><?php echo $golf_configuration['main_title']; ?></h1>
							<h5><?php echo $golf_configuration['sub_title']; ?></h5>
							<div>
								<p class="spcl_txt"><?php echo $golf_configuration['boxed_title']; ?></p>
							</div>
						</div>
					</div>
				</div>
	...
	<?php } ?>
	<?php if(isset($golf_configuration['banner_dsc']) && !empty($golf_configuration['banner_dsc'])){?>
	<section class="transport_blk">
		<section class="container">
			<section class="row">
				<div class="col-sm-12">						
					<div class="form_text_blk no-shadow">
						<h3><?php echo $golf_configuration['banner_dsc']; ?></h3>
					</div>
				</div>
			</section>
		</section>
							...
							<div class="filter_search_blk">
									...
									<?php if ($search_title) { ?>
										<h2><?php echo $text_Search;?></h2>
								   				<?php if (isset($total_golfs)) { ?>
                                					<?php if ($total_golfs > 1) { ?>
                                   	 				<?php echo $total_golfs; ?> <?php echo $text_results;?>
                               						<?php } else if ($total_golfs == 1) { ?>
                                  							  <?php echo $total_golfs; ?> <?php echo $text_result;?>
                                							<?php } ?>
                            					<?php } else { ?>
								...
								<div class="single_list_blk">
									<div class="tourimg">
										<?php if(isset($golf['image']) && file_exists($golf['image'])) { ?>
											<?php 
												  echo $optimized->resize($golf['image'], 230, 153, array(), 'resize_crop');
											?>
										<?php } else { ?>
									...
									<div class="tour_short_detail">
										...
										<p>
											...
											<?php 
												if(!empty($golf['dsc'])) { 
													$golf_dsc = strip_tags($golf['dsc']);
													
													$short_golf_dsc = substr($golf_dsc, 0, 120);
													echo $short_golf_dsc . THREE_DOTS;
												} else {
													echo THREE_DASH;
												}
											?>
									...
									<div class="price_detail">
										<h4><?php echo $text_from_usd;?></h4>
											<h2> $<?php echo number_format($golf['price'], PRICE_DELIMITER); ?></h2>           							
										<a href="<?php echo base_url('golfs/' . $golf['country_seo_url'] . '/' . $golf['city_seo_url'] . '/' . $golf['slug']); ?>"><?php echo $text_Book_Now;?></a>
									</div>
								</div>
							<?php } ?>
