...
class Tour_deals_mdl extends CI_Model {
    ...
    public function get_tours($search_conditions = array(), $start, $limit) {
		...
		$where = '(t1.adult_deal_price!="0" or t1.child_deal_price!="0")';
		$this->db->where($where);
        ...
        if(isset($search_conditions['search_min_price']) && !empty($search_conditions['search_min_price'])) {
            $this->db->where('t1.adult_deal_price >=', $search_conditions['search_min_price']);
        }
        
        if(isset($search_conditions['search_max_price']) && !empty($search_conditions['search_max_price'])) {
            $this->db->where('t1.adult_deal_price <=', $search_conditions['search_max_price']);
        }
    ...
    public function get_tour_banners() {
        $this->db->select('*')->from('tv_banner_tbl')->where('category', 'deals');
