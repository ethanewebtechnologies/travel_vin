...
class Restaurant_mdl extends CI_Model {
    ...
    public function get_restaurants($search_conditions = array(), $start, $limit) {
        ...
        $this->db->from("tv_restaurant_tbl t1");
        $this->db->join("tv_restaurant_details_tbl t2", "t1.id = t2.restaurant_id", "left");
		...
		if(isset($search_conditions['search_category_id']) && !empty($search_conditions['search_category_id'])) {
            $this->db->where('t1.restaurant_category_id', $search_conditions['search_category_id']);
        }
    ...
    public function get_total_restaurants($search_conditions = array()) {
        
        $this->db->select("count(t1.id) as total");
        $this->db->from("tv_restaurant_tbl as  t1");
        $this->db->join("tv_restaurant_details_tbl t2", "t1.id = t2.restaurant_id", "left");
    ...
    /* public function get_restaurant_images_by_restaurant_ids($restaurant_ids_array) {
     $this->db->select("image, restaurant_id");
     $this->db->from("tv_restaurant_images_tbl");
     $this->db->where_in('restaurant_id', $restaurant_ids_array);
     $query = $this->db->get();
     return $query->result_array();
     } */
    ...
    public function get_restaurant_images_by_restaurant_ids($restaurant_ids_array) {
        ...
        $this->db->where_in('item_id', $restaurant_ids_array);
        $query = $this->db->get();
        return $query->result_array();
    }
    
    // For Get restaurant details
    public function get_restaurant_detail($search_conditions = array()) {
        return $this->db->select("t1.id as restaurant_id, t1.*, t2.*")
        ->from("tv_restaurant_tbl as t1")
        ->join("tv_restaurant_details_tbl as t2", "t2.restaurant_id = t1.id", "left")
        ->where("t1.slug", $search_conditions['restaurant_slug'])
        ->where("t2.language_code", $search_conditions['language_code'])
        ->get()->row_array();
    }
    
    /* public function get_block_dates($restaurant_id) {
     $this->db->select("*")->from("tv_restaurant_block_booking_dates_tbl")->where("restaurant_id", $restaurant_id);
     $query = $this->db->get();
     return $query->result_array();
     } */
    
    public function get_block_dates($restaurant_id) {
        $this->db->select("*")->from("tv_block_booking_dates_tbl")->where("item_type", 'restaurant')->where("item_id", $restaurant_id);
        $query = $this->db->get();
        return $query->result_array();
    }
    ...
    public function get_restaurant_banners(){
        $this->db->select('*')->from('tv_banner_tbl')->where('category','restaurant');
        $query = $this->db->get();
