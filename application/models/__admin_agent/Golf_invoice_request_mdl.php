...
/*
 * APPLICATION 		: Golf Invoice Request Model
 * AUTHOR			: KUNDAN KUMAR
...
class Golf_invoice_request_mdl extends CI_Model {
    public function __construct() {
        parent::__construct();
    }
    
    public function get_golf_bookings($conditions = array(), $start, $limit) {
        ...
        $this->db->where('t1.booking_type', 'golf');
		...
		if (isset($conditions['search_golf_invoice_date']) && !empty($conditions['search_golf_invoice_date'])) {
			$this->db->where('t2.date_generated', lu_to_d($conditions['search_golf_invoice_date']));
		}
		...
		if (isset($conditions['sort_golf_invoice_date']) && !empty($conditions['sort_golf_invoice_date'])){
			$this->db->order_by('t2.date_generated', $conditions['sort_golf_invoice_date']);
		}
    ...
    public function get_total_golf_bookings($conditions = array()) {
    ...
    public function get_golf_booking($booking_id) {
        $this->db->select("t1.*, t2.firstname, t2.lastname")
        ->from("tv_golf_bookings_tbl as t1")
    ...
    public function update_golf_booking($post_data, $golf_booking_id) {
        ...
        $this->db->where('booking_id', $golf_booking_id);
    ...
    public function disable_golf_booking($golf_booking_id) {
        $data = array(
            'status' => 0
        );
        
        $this->db->where('booking_id', $golf_booking_id);
        $this->db->update("tv_golf_bookings_tbl", $data);
        
        return true;
    }
    
    public function update_golf_booking_customer_details($data, $customer_id) {
    ...
    public function get_invoice_data_by_booking_ids($booking_ids) {
        $this->db->select("t2.*, t1.agent_id, t1.agent_cost, t1.country_id, t1.city_id, t3.invoice_no, t3.date_generated, t4.booking_date, t4.booking_no")
            ->from("tv_golf_tbl as t1")
                ->join("tv_golf_booking_tbl as t2", "t1.id = t2.golf_id" ,"left")
