...
class Club_and_bar_invoice_request_mdl extends CI_Model {
    public function __construct() {
        parent::__construct();
    }
    
    public function get_club_and_bar_bookings($conditions = array(), $start, $limit) {
        ...
        $this->db->where('t1.booking_type', 'club_and_bar');
        ...
        if (isset($conditions['search_club_and_bar_invoice_date']) && !empty($conditions['search_club_and_bar_invoice_date'])) {
            $this->db->where('t2.date_generated', lu_to_d($conditions['search_club_and_bar_invoice_date']));
        }
        ...
        if (isset($conditions['sort_club_and_bar_invoice_date']) && !empty($conditions['sort_club_and_bar_invoice_date'])){
            $this->db->order_by('t2.date_generated', $conditions['sort_club_and_bar_invoice_date']);
        }
    ...
    public function get_total_club_and_bar_bookings($conditions = array()) {
    ...
    public function get_club_and_bar_booking($booking_id) {
        $this->db->select("t1.*, t2.firstname, t2.lastname")
                  ->from("tv_club_and_bar_bookings_tbl as t1")
    ...
    public function update_club_and_bar_booking($post_data, $club_and_bar_booking_id) {
        ...
        $this->db->where('booking_id', $club_and_bar_booking_id);
    ...
    public function disable_club_and_bar_booking($club_and_bar_booking_id) {
        $data = array(
            'status' => 0
        );
        
        $this->db->where('booking_id', $club_and_bar_booking_id);
        $this->db->update("tv_club_and_bar_bookings_tbl", $data);
        
        return true;
    }
    
    public function update_club_and_bar_booking_customer_details($data, $customer_id) {
    ...
    public function get_invoice_data_by_booking_ids($booking_ids) {
        $this->db->select("t2.*, t1.agent_id, t1.agent_cost, t1.country_id, t1.city_id, t3.invoice_no, t3.date_generated, t4.booking_date, t4.booking_no")
             ->from("tv_club_and_bar_tbl as t1")
                 ->join("tv_club_and_bar_booking_tbl as t2", "t1.id = t2.club_and_bar_id" ,"left")
