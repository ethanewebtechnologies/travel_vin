...
/*
 * APPLICATION 		: Restaurant Invoice Request Model
 * AUTHOR			: KUNDAN KUMAR
...
class Restaurant_invoice_request_mdl extends CI_Model {
    ...
    public function get_restaurant_bookings($conditions = array(), $start, $limit) {
        ...
        if (isset($conditions['search_restaurant_invoice_date']) && !empty($conditions['search_restaurant_invoice_date'])) {
            $this->db->where('t2.date_generated', lu_to_d($conditions['search_restaurant_invoice_date']));
        }
        ...
        if (isset($conditions['sort_restaurant_invoice_date']) && !empty($conditions['sort_restaurant_invoice_date'])){
            $this->db->order_by('t2.date_generated', $conditions['sort_restaurant_invoice_date']);
        }
    ...
    public function get_restaurant_booking($booking_id) {
        ...
        ->from("tv_restaurant_bookings_tbl as t1")
    ...
    public function get_invoice_data_by_booking_ids($booking_ids) {
        ...
        ->join("tv_restaurant_booking_tbl as t2", "t1.id = t2.restaurant_id" ,"left")
