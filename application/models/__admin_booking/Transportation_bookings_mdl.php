...
class Transportation_bookings_mdl extends CI_Model {
    ...
    public function get_transportation_bookings($conditions = array(), $start, $limit) {
		$this->db->select("t1.*,t2.flight_arrival_date,t2.flight_arrival_time,t2.flight_departure_date,t2.flight_departure_time, t3.company_legal_name,t4.firstname,t4.lastname")
		...
		if (isset($conditions['search_arrival_date']) && !empty($conditions['search_arrival_date'])) {
		   
		    $this->db->like('date(t2.flight_arrival_date)', lu_to_d_for_date($conditions['search_arrival_date']));
		    $this->db->or_like('date(t2.flight_departure_date)', lu_to_d_for_date($conditions['search_arrival_date']));
		}
		...
		if (isset($conditions['sort_arrival_date']) && !empty($conditions['sort_arrival_date'])) {
			$this->db->order_by('t2.flight_arrival_date', $conditions['sort_arrival_date']);
		}
		if (isset($conditions['sort_dept_date']) && !empty($conditions['sort_dept_date'])) {
			$this->db->order_by('t2.flight_departure_date', $conditions['sort_dept_date']);
		}
    ...
    public function disable_transportation_booking($transportation_booking_id) {
        ...
        $this->db->update("tv_transportation_bookings_tbl", $data);
        
        return true;
    }
    
    public function update_transportation_booking_customer_details($data, $customer_id) {
