...
/*
 * APPLICATION 		: Tour Booking  Model
...
class Tour_booking_mdl extends CI_Model {
    ...
    public function get_tour_bookings($conditions = array(), $start, $limit) {
        ...
        $this->db->select("t1.*,t2.tour_start_date,t2.pickup_time, t3.company_legal_name,t4.firstname,t4.lastname")
                ->from('tv_bookings_tbl as t1')
                   ->join("tv_tour_bookings_tbl as t2", 't2.booking_id=t1.booking_id')
         ...
         if (isset($conditions['search_costumer_name']) && !empty($conditions['search_costumer_name'])) {
          ...
          if (isset($conditions['search_tour_start_date']) && !empty($conditions['search_tour_start_date'])) {
              $this->db->where('date(t2.tour_start_date)', lu_to_d_for_date($conditions['search_tour_start_date']));
          }
          ...
          if (isset($conditions['sort_tour_start_date']) && !empty($conditions['sort_tour_start_date'])) {
              $this->db->order_by('t2.tour_start_date', $conditions['sort_tour_start_date']);
          }
