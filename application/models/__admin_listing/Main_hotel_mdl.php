...
/*
 * APPLICATION 		: Main Hotel Model
 * AUTHOR			: Kundan Kumar
 * VERSION			: 1.0
 * COPYRIGHT		: Ethane Webtechnologies Pvt. Ltd.
 */

class Main_hotel_mdl extends CI_Model {

    public function __construct() {
        parent::__construct();
    }

    public function get_hotels($conditions = array(), $start, $limit) {
    ...
    public function get_total_hotels($conditions = array()) {
    ...
    public function get_hotel($hotel_id) {
        ...
        $query = $this->db->select('*')->from('tv_location_tbl')->where('id', $hotel_id)->get();
        return $query->row_array();
    }

    public function myhotel($hotel_id) {
        $query = $this->db->select('*')->from('tv_location_tbl')->where('id', $hotel_id)->get();
        return $query->row_array();
    }
